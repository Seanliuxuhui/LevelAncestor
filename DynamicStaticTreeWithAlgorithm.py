...
class DynamicMacroMicroTree:
    def __init__(self, R, n, maxlength):
        ...
        self.n = n
        self.M = int(0.5 * math.pow(2, self.r0))
        self.ranks = [-1] * self.n
        self.R = R
    ...
    def build_datastructure(self):
        ...
        self.bitindex, self.nodetable, self.anc = self._process_micro_tree()
    ...
    def rank(self, v, depth, size, maxdepth):
        ...
        idx = 0
        while idx < maxdepth:
            if depth[v] % int(math.pow(2, idx)) == 0:
                idx += 1
            else:
                return idx
    ...
    def _preprocess_macro_trees(self):
        ...
        levelanc = {node[i]: [-1] * (ranks_macro_node[node[i]] + 1) for i in range(node_idx)}
        for i in range(node_idx):
            v = node[i]
            if v != -1 and parents[v] != -1:
                for j in range(ranks_macro_node[v]):
    ...
    def _dfs_micro_tree(self):
        ...
        return jump
    def _process_micro_tree(self):
        N = int(0.5 * math.log(n, 2))
        bitindex = [[0] * N for j in range(int(math.pow(2, N)))]
        bitindex[0] = [-(i + 1) for i in range(N)]
        for j in range(N):
            for k in range(int(math.pow(2, j))):
                bitindex[int(math.pow(2, j)) + k][0] = j
                for i in range(1, N):
                    bitindex[int(math.pow(2, j)) + k][i] = bitindex[k][i - 1]
        self.microset_idx = 0
        nodetable = collections.defaultdict(list)
        anc = [-1] * self.n
        for node in self.tree_node_visit_sequence:
            self.d[node] = 1
            if node in self.edges:
                ...
                for w in self.edges[node]:
                    if self.d[node] + self.d[w] > (self.M + 1) / 2:
                        self.microtree_root_table[self.microset_idx] = node
                        nodetable[self.microset_idx].append(node)
                        self.microset_size[self.microset_idx] += 1
                        anc[node] = int(pow(2, 0))
                        i = 1
                        stack = []
                        visited = []
                        for v in self.edges[node]:
                            if v == w:
                                break
                            stack.insert(0, v)
                            nodetable[self.microset_idx].insert(i, v)
                            anc[v] = anc[node] + int(math.pow(2, i))
                            i += 1
                            visited.append(v)
                        for each_node in visited:
                            self.edges[node].remove(each_node)
                        stack.insert(0, w)
                        nodetable[self.microset_idx].insert(i, w)
                        anc[w] = anc[node] + int(math.pow(2, i))
                        i += 1
                        self.edges[node].remove(w)
                        ...
                        while stack:
                            ...
                            if v in self.edges:
                                for each_child in self.edges[v]:
                                    nodetable[self.microset_idx].insert(i, each_child)
                                    anc[each_child] = anc[v] + int(math.pow(2, i))
                                    i += 1
                                    stack.insert(0, each_child)
                                del self.edges[v]
                        self.d[node] = 1
        ...
        for v, edges in self.edges.items():
            if len(edges) > 0 and v != 0:
                self.microtree_root_table[self.microset_idx] = v
                nodetable[self.microset_idx].append(v)
                anc[v] = int(math.pow(2, 0))
                i = 1
                stack = edges
                while stack:
                    w = stack.pop(0)
    ...
    def _level_ancestor_micro(self, v, x):
        i = self.bitindex[self.anc[v]][x]
        if i > -1:
            return self.nodetable[self.node_belongs_to[v]][i]

    def get_depth(self, i):
        return self.depth[i]
    def _add_leaf(self, v, p):
        new_microset = False
        k = self.microset_size[self.node_belongs_to[p]]
        if k >= self.M:
            k = 0
            self.node_belongs_to[v] = self.microset_idx
            self.microtree_root_table[self.microset_idx] = v
            new_microset = True
            self.nodetable[self.node_belongs_to[v]].insert(k, v)
            self.microset_idx += 1
        else:
            self.node_belongs_to[v] = self.node_belongs_to[p]
            self.nodetable[self.node_belongs_to[p]].insert(k, v)
        self.depth[v] = self.depth[p] + 1
        if new_microset:
            self.anc[v] = int(math.pow(2, 0))
        else:
            self.anc[v] = self.anc[p] + int(math.pow(2, k))
    ...
    def add_leaf(self, v, p):
        self.froms[v] = p
        new_microset = self._add_leaf(v, p)
        if self.depth[p] % self.M == 0:
            self.jumpM[v] = p
        else:
            self.jumpM[v] = self.jumpM[p]

        if new_microset:
            self.levelancM[v] = [v, p] + self.levelancM[p][1: self.M+1]
        else:
...
macro_micro_tree.constructGraph(1, 0, 0, R)
macro_micro_tree.build_datastructure()
# macro_micro_tree.add_leaf(5, 2)
# macro_micro_tree.add_leaf(3, 5)
# macro_micro_tree.add_leaf(4, 5)
# macro_micro_tree.add_leaf(1, 5)
# macro_micro_tree.add_leaf(7, 1)
# macro_micro_tree.add_leaf(9, 1)
# macro_micro_tree.add_leaf(10, 9)
# macro_micro_tree.add_leaf(11, 10)
# macro_micro_tree.add_leaf(6, 10)
# macro_micro_tree.add_leaf(8, 10)
...
#     macro_micro_tree.constructGraph(i, t, i, R)
#     t = i
...
# macro_micro_tree.add_leaf(37, 9)
# macro_micro_tree.add_leaf(38, 9)
# macro_micro_tree.add_leaf(39, 9)
# macro_micro_tree.add_leaf(40, 37)
...
# print(results)
# print(macro_micro_tree.level_ancestor(40, 8))
# print(macro_micro_tree.level_ancestor(6, 5))